...
Fistly, define variables like `LIBDO_LOG`.

Then, use `. /PATH/TO/LIBDO` to import libdo.
(Command `source` is equal to command `.`)

Lastly, use `DO cmd` to `execute `cmd`.

Please pay attention that program `eval` is used to execute programs.
So, if you want to use redirections, pipes or quotes, escape them first.

== ENVIRONMENTAL VARIABLES

* `LIBDO_TOP_PID` the `pid` to kill if failed.
If blank, the program will not kill any process.

* `LIBDO_PRIV` exit status.

* `LIBDO_LOG` log name.
If blank, there will be no logs.
...
-rwxrwxr-x+ 1 admin None 9.5K Jun  5 00:46 install.sh*
...
Please be aware that we __DO NOT__ recommend using LibDO interactively.
You may use it in a shell script.
