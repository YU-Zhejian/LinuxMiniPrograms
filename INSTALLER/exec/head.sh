...
if [ -z "${myhead:-}" ]; then
    ...
    if [ -z "${myhead:-}" ]; then
        if [ -z "${lntmp:-}" ]; then
            ...
            echo -e "\e[30mERROR: head still not found. Please configure it manually in LMP_ROOT/etc/"${path_sh}".\e[0m"
        else
            echo -e "\e[30mWARNING: Will use BSD head.\e[0m"
