...
if [ -z "${mymv:-}" ]; then
    ...
    if [ -z "${mymv:-}" ]; then
        if [ -z "${lntmp:-}" ]; then
            ...
            echo -e "\033[31mERROR: mv still not found. Please configure it manually in LMP_ROOT/etc/"${path_sh}".\033[0m"
        else
            echo -e "\033[31mWARNING: Will use BSD mv.\033[0m"
            echo "mymv=\"${lntmp}\" #${type}" >>"${path_sh}"
        fi
    fi
    unset mv_ver line
else
    echo -e "\033[033mmv configured\033[0m"
fi
