...
if [ -z "${mycut:-}" ]; then
    ...
    if [ -z "${mycut:-}" ]; then
        if [ -z "${lntmp:-}" ]; then
            ...
            echo -e "\e[30mERROR: cut still not found. Please configure it manually in LMP_ROOT/etc/"${path_sh}".\e[0m"
        else
            echo -e "\e[30mWARNING: Will use BSD cut.\e[0m"
