...
* `--install-man` Install doc in Groff man to `man`, need `asciidoctor`.

* `--install-usage` Install yldoc usage to `usage`, need python 3.

* `--install-pdf` Install doc in pdf to `pdf`, need `asciidoctor-pdf`.

* `--install-html` Install doc in html to `html`, need `asciidoctor` or `asciidoc`.
