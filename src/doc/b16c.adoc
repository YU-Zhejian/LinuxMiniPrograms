...
`b16c` will read from standard input and pumps what is encoded/decoded to standard output.

== USAGE

`b16c` is firstly integrated into `enigma`. It is later separated and developed into C and Python version.
...
`b16c` can also be used with `enigma`.
