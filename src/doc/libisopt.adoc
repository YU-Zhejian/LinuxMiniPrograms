...
Firstly, use `. /PATH/TO/LIBISOPT` to import libisopt.
...
After this loop, options and other parameters are separated into string `"${opt}"` and `${STDS}`.
If there're unrecognized option, the program will exit with exit code 1.
