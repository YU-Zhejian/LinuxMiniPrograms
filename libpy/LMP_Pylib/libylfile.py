...
def ylread(filename: str) -> str:
	...
	:param filename: The file you need to read. Use '-' or '/dev/stdin' to read from standard input.
...
def mktemp(S: str) -> str:
	"""
	A function used to replace 'mktemp' under GNU/Linux. Can create temporary files under /tmp. See 'man mktemp' for more details.
	:param S: The path template. e.g. libdo_man.XXXXXX The tailing X will be replaced by random numbers.
	:return: The path of temporary file.
	WARNING: This function DO NOT remove the file it creates. You need to remove it manually.
	"""
...
def pywcl(filename: str) -> int:
	"""
	A function used to replace 'wc -l' under GNU/Linux. Have the capacity to read large files.
	:param filename: The filename you need to read Use '-' or '/dev/stdin' to read from standard input.
	:return: Line numbers.
	"""
	count = 0
	f = open(filename, "r")
