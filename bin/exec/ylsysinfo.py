...
if os.path.isfile("/etc/passwd"):
    print("\033[033mUser information generated by '/etc/passwd':\033[0m")
    tbl=["#1","#1","#1","#1","#S50","#1","#1",'User name;User effect group names;User ID;User initial group ID;User gecos;User home directory;User Shell']
    tmptbl=ylread("/etc/passwd").split('\n')
    for item in tmptbl:
        item=item.split(':')
        grph=os.popen("groups "+item[0])
        grp=re.findall(r':.*',grph.read().strip())[0]
        grph.close()
        item[1]=grp
        tbl.append(';'.join(item))
    tbl.extend(tmptbl)
    ylwrite(tmpf,'\n'.join(tbl))
    mktbl(tmpf)
else:
    ...
    if not os.system("id -a &>> /dev/null || id &>> /dev/null"):
        print("\033[033mUser information generated by 'id -a||id':\033[0m")
        verstr = yldo("id -a|id")
        print(verstr)
        verstrs = verstr.split(' ')
        tbl = ["#1", "#1"]
...
if not os.system("mount &>> /dev/null"):
    print("\033[033mFile system information generated by 'mount':\033[0m")
    tbl = ["#1", "#1", "#1", "#S50","Devise;Mount point;Type;Info"]
