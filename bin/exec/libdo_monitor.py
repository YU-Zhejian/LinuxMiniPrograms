...
while True:
	...
	print(now() + 'CurrentMEM: ' + ','.join(
		['Total=' + str(cmem.total), 'ImmAvail=' + str(cmem.available), 'AvailPer=' + str(cmem.percent),
		 'Buffer=' + str(cmem.buffers), 'Cached=' + str(cmem.cached), 'Shared=' + str(cmem.shared)]))
	cmem = psutil.swap_memory()
	print(now() + 'CurrentSWAP: ' + 'Total=' + str(cmem.total) + ',ImmAvail=' + str(cmem.free) + ',AvailPer=' + str(
		cmem.percent))
	tsp = now()
	try:
		...
		for subp in ptable:
			print(tsp + str(subp.pid) + ':INFO:' + ','.join(
				["EXE=" + subp.exe(), "CMD=" + ' '.join(subp.cmdline()), "STAT=" + subp.status(), "CWD=" + subp.cwd(),
				 "OnCPU=" + str(subp.cpu_num()), "%CPU=" + str(subp.cpu_percent(0.2)), "NICE=" + str(subp.nice())]))
			print(tsp + str(subp.pid) + ':MEM:' + ','.join(
				["Residential=" + str(subp.memory_info().rss), "Virtual=" + str(subp.memory_info().vms),
				 "Shared=" + str(subp.memory_info().shared), "text=" + str(subp.memory_info().text),
				 "data=" + str(subp.memory_info().data)]))
			print(tsp + str(subp.pid) + ':IO:' + yldo(
				'cat /proc/' + str(subp.pid) + r'''/io |tr '\n' ','|sed 's;: ;=;g' '''))
			...
			for x in subp.open_files():
				print(
					tsp + str(subp.pid) + ':FILES:' + ','.join(["PATH=" + x.path, "FD=" + str(x.fd), "MODE=" + x.mode]))
			for x in subp.threads():
				subt = psutil.Process(x.id)
				print(tsp + str(subp.pid) + ':THREADS:' + ','.join(
					["PID=" + str(subt.pid), "OnCPU=" + str(subt.cpu_num())]))
				print(tsp + str(subp.pid) + ':IO:PID=' + str(subt.pid) + ',' + yldo(
					'cat /proc/' + str(subt.pid) + r'''/io |tr '\n' ','|sed 's;: ;=;' '''))
